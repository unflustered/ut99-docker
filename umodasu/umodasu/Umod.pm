...
sub open {
	...
	foreach my $requirement ( $ini->get( ['Setup', 'Requires'],
	    -mapping => 'multiple' ) ) {
            my %hash = (
		    product => $ini->get( [$requirement, 'Product'], -mapping => 'single' ),
		    version => $ini->get( [$requirement, 'Version'], -mapping => 'single' ),
		    localproduct => $ini->get( [$requirement, 'LocalProduct'], -mapping => 'single' ),
		    producturl => $ini->get( [$requirement, 'ProductURL'], -mapping => 'single' ),
		    versionurl => $ini->get( [$requirement, 'VersionURL'], -mapping => 'single' ),
		    developer => $ini->get( [$requirement, 'Developer'], -mapping => 'single' ),
		    developerurl => $ini->get( [$requirement, 'DeveloperURL'], -mapping => 'single' )
	    );
